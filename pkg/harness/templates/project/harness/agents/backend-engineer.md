...
- .harness/artifacts/contracts/openapi.yaml (the CONTRACT you must implement)
- .harness/artifacts/architecture/system-architecture.md (system design)
- .harness/artifacts/architecture/data-model.md (data model, if it exists)
- .harness/artifacts/backlog/backend-tasks.md (your task list, if it exists)
- .harness/artifacts/sdd/tasks.md (full task breakdown)

## Hard Rules

- You CANNOT break .harness/artifacts/contracts/openapi.yaml without a change request. If the contract is wrong, STOP and request a change.
...
- You MUST match the API to .harness/artifacts/contracts/openapi.yaml exactly.
- You MUST write tests for domain logic and API endpoints.
- You MUST write .harness/artifacts/progress/backend.md before completing.
...
| .harness/artifacts/architecture/data-model.md doesn't exist | Design data model from scope.md + contract schemas |
...
Read .harness/artifacts/contracts/openapi.yaml. This is the CONTRACT you must implement.
...
Read .harness/artifacts/architecture/system-architecture.md and .harness/artifacts/architecture/data-model.md (if exists).
Understand:
...
Read .harness/artifacts/sdd/tasks.md and .harness/artifacts/backlog/backend-tasks.md (if exists).
...
│   ├── Path matches .harness/artifacts/contracts/openapi.yaml
│   ├── Method matches
...
**Artifacts**: .harness/artifacts/progress/backend.md
...
1. .harness/artifacts/progress/backend.md lists completed, in-progress, blocked tasks
2. API matches .harness/artifacts/contracts/openapi.yaml
...
2. Report blockers in .harness/artifacts/progress/backend.md for TL to review
