...
func configEnv(args []string) {
	...
	fmt.Println()
	fmt.Println("Stitch:")
	fmt.Println("  STITCH_API_KEY                    Google Stitch API key")
	fmt.Println("  STITCH_ACCESS_TOKEN               OAuth/access token alternative")
	fmt.Println("  GOOGLE_CLOUD_PROJECT              Project required with STITCH_ACCESS_TOKEN")
	fmt.Println("  SHIPWRIGHT_STITCH_API_KEY_ENV     Rename the env var Shipwright reads for Stitch API key")
	fmt.Println("  SHIPWRIGHT_STITCH_ACCESS_TOKEN_ENV Rename the env var Shipwright reads for Stitch access token")
	fmt.Println("  SHIPWRIGHT_STITCH_PROJECT_ENV     Rename the env var Shipwright reads for Google Cloud project")
	fmt.Println("  SHIPWRIGHT_STITCH_MCP_URL         Stitch MCP endpoint override")
	fmt.Println()
	fmt.Println("OpenDesign:")
	fmt.Println("  OPENDESIGN_MCP_COMMAND         OpenDesign MCP command, usually node")
	fmt.Println("  OPENDESIGN_MCP_ARGS            Arguments separated by comma or |")
	fmt.Println("  OPENDESIGN_DATA_DIR / OD_DATA_DIR")
	fmt.Println("  OPENDESIGN_SIDECAR_IPC_PATH / OD_SIDECAR_IPC_PATH")
	fmt.Println()
