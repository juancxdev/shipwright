...
## Evidence-first design contract

Before using Stitch, OpenDesign, OpenPencil, or doc-only redesign output on an existing frontend, you MUST complete the baseline-web provider task from `.harness/artifacts/design/baseline/design-task.md`.

Hard rules:

- Do not replace the project logo, icons, screenshots, imagery, typography, brand colors, or meaningful copy unless the user explicitly approves the change.
- Write `.harness/artifacts/design/asset-manifest.json` and mark real logo assets with `role: "logo"`.
- Capture or reference source screenshots under `.harness/artifacts/design/source-screenshots/` before claiming fidelity.
- Provider outputs are NOT the source of truth; the rendered existing app and asset manifest are.
- UX cannot be approved if `baseline-captured`, `assets-preserved`, `provider-published`, `fidelity-verified`, or `token/quota-ok` is blocked.
- If OpenDesign/Stitch reports quota, tokens exhausted, publish failed, import failed, or partial, write it honestly and stop; do not retry until costs/quota are understood.
...
4. Use OpenDesign MCP for artifact work, not canvas node/frame work. The OpenCode config must expose OpenDesign at `.opencode/opencode.json` under `mcp.open-design`. Do not ask the user to edit `.opencode/opencode.json`. If missing, ask them to run `shipwright integrations configure opendesign --help` and `shipwright executor generate opencode`, then restart OpenCode.
...
7. For existing UI baselines, compare OpenDesign artifact screenshots/exports against source screenshots and the asset manifest before proposing redesigns. If the logo or brand assets differ, status is fail unless `.harness/approvals/design-asset-change.json` exists.
