...
Do not use OpenPencil unless the user explicitly asks for OpenPencil. For existing apps, read `.harness/artifacts/design/baseline/design-task.md` and `.harness/artifacts/design/asset-manifest.json` before generating; preserve real logos/assets/copy and fail fidelity if they are substituted. Stitch is preferred when the goal is high-fidelity UI generation, variants, screenshots, HTML export, and design-to-development evidence.
