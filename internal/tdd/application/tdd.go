...
func RefreshTDDPolicyFromProfile(profile *projectprofile.ProjectProfile) (*TDDPolicy, error) {
...
func BuildTDDPolicy(profile *projectprofile.ProjectProfile) *TDDPolicy {
	policy := &TDDPolicy{
		Version:     TDDPolicyVersion,
		GeneratedAt: nowISO(),
		Mode:        TDDModeNone,
		Source:      projectprofile.ProjectProfileJSON,
	}
	if profile == nil {
...
func FormatTDDAssessment(assessment TDDAssessment) string {
	...
	sb.WriteString("\n\n")
	sb.WriteString(fmt.Sprintf("Policy file:      %s\n", formatYesNo(assessment.PolicyPresent)))
	sb.WriteString(fmt.Sprintf("Mode:             %s\n", assessment.Mode))
	sb.WriteString(fmt.Sprintf("Supported:        %s\n", formatYesNo(assessment.Supported)))
